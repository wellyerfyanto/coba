...
// PERBAIKAN BESAR: Fungsi untuk setup proxy dengan support multiple proxies - DIPERBAIKI!
async function setupProxy(config, sessionId, socketId, sessionIndex) {
  ...
  // OPTION 3: Multiple manual proxies (dipisah koma) - DIPERBAIKI!
...
async function runBotSession(config, sessionIndex, mainSessionId, socketId = null) {
  ...
  try {
    ...
    // 3. Setup Proxy dengan support multiple - DIPANGGIL DI SINI
...
// PERBAIKAN: Fungsi utama untuk menjalankan semua sesi bot - DITAMBAH ERROR HANDLING
  ...
  try {
    ...
    for (let i = 0; i < sessionCount; i++) {
      try {
        ...
        // Jika gagal, lanjut ke session berikutnya atau stop berdasarkan konfig
        if (config.stopOnError) {
          throw sessionError; // Stop semua sessions
        }
        // Kalau tidak, lanjut saja
      }
    }
...
// Socket.IO connection - SATU-SATUNYA TEMPAT UNTUK HANDLE 'start-bot'
io.on('connection', (socket) => {
  console.log(`[SERVER] 👤 Client connected: ${socket.id}, Total clients: ${io.engine.clientsCount}`);
  
  socket.emit('connected', { 
    socketId: socket.id,
    message: 'Connected to bot server v3.0',
    timestamp: new Date().toISOString()
  });
  
  // LOG SETIAP KALI 'start-bot' DITERIMA
  socket.on('start-bot', async (config) => {
    console.log(`[SERVER] 🎯 Received "start-bot" from ${socket.id}`);
    console.log(`[SERVER] Config: Target=${config.target}, Sessions=${config.sessionCount || 1}, ProxySource=${config.proxySource}`);
    ...
    try {
      ...
      console.log(`[SERVER] Bot session ${config.sessionId} starting...`);
