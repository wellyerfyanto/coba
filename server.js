...
async function getBrowserLaunchOptions(config, sessionId) {
  // Tentukan executable path untuk Railway
  let executablePath;
...
// PERBAIKAN BESAR: Fungsi untuk setup proxy dengan support multiple proxies
...
// Fungsi untuk traffic website - DIPERBAIKI!
  ...
  try {
    ...
    for (let i = 0; i < scrollConfig.count; i++) {
      ...
      emitStatus(socketId, {
        ...
        progress: 30 + Math.floor((i / scrollConfig.count) * 30)
      });
    }
    ...
    if (visitTime > 0) {
      ...
      for (let i = 0; i < staySegments; i++) {
        ...
        emitStatus(socketId, {
          ...
          progress: 70 + Math.floor((i / staySegments) * 25)
        });
      }
    }
...
// PERBAIKAN: Fungsi utama untuk menjalankan semua sesi bot
...
io.on('connection', (socket) => {
  ...
  socket.on('start-bot', async (config) => {
    ...
    console.log(`[SERVER] 📊 Config details:`, {
