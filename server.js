...
const shutdown = (signal) => {
    ...
    // Stop bot safely - FIXED: Handle both Promise and non-Promise returns
    if (botModule && typeof botModule.stop === 'function') {
        try {
            // Get the result from stop()
            ...
            if (stopResult && typeof stopResult.then === 'function') {
                ...
                stopResult.then(() => {
                    console.log('✅ Bot stopped successfully (async)');
                }).catch(err => {
                    console.error('⚠️ Error stopping bot (async):', err.message);
                });
            } else {
                // It's not a promise, just log
                console.log('✅ Bot stopped successfully (sync)');
            }
        } catch (error) {
            console.error('❌ Error in stop() function:', error.message);
        }
    } else {
        console.log('ℹ️ No bot module or stop function found');
    }
