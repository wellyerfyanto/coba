...
// ======================
// 4. LOAD BOT MODULES (FIXED: No circular dependency)
// ======================
console.log('📦 Loading bot modules...');

let botModule, proxyModule, validatorModule;

// Try to load bot module (FIXED PATH: using ./libs/index)
try {
    botModule = require('./libs/index'); // PERHATIAN: Path sudah diperbaiki
    console.log('✅ Bot module loaded');
    
    // Show bot configuration status
    if (botModule.getStatus) {
        const status = botModule.getStatus();
        console.log(`🤖 Bot Status: ${status.advancedAvailable ? 'Advanced AVAILABLE' : 'Advanced NOT available'}`);
        console.log(`🤖 Platform: ${status.platform}, Mode: ${status.mode}`);
    }
} catch (error) {
    ...
    server = app.listen(PORT, '0.0.0.0', () => {
        ...
        if (botModule.getStatus) {
            ...
            if (!status.advancedAvailable) {
                ...
                console.log('ℹ️  Check if behavior-engine.js and libs_advanced-http.js exist in /libs/ folder');
            }
        }
