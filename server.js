...
const ProxyFetcher = require('./utils/proxyFetcher');  // ✅ TAMBAHAN
...
app.get('/api/debug', (req, res) => {
  res.json({
    ...
    environment: {
      ...
      chromium: process.env.CHROMIUM_PATH || 'not set',
      puppeteer: puppeteer.version
    },
    server: {
      uptime: process.uptime(),
...
app.get('/api/status', (req, res) => {
  res.json({
    success: true,
    message: 'Traffic Bot API v3.2 (Multi-Browser) is running',
    timestamp: new Date().toISOString(),
    uptime: process.uptime(),
    memory: process.memoryUsage(),
    activeSessions: activeSessions.size,
...
// API untuk mendapatkan info browser yang tersedia
  ...
  res.json({
    ...
    browserConfig: {
      ...
      firefox: {
        ...
        status: isRailway ? 'available' : 'requires-installation'
      },
      webkit: {
        executablePath: null,
        status: 'coming-soon'
...
// ==================== PROXY FETCHER ENDPOINTS ====================

// Fetch proxies from multiple sources
app.get('/api/fetch-proxies', async (req, res) => {
  try {
    console.log('🌐 [SERVER] Fetching proxies from multiple sources...');
    
    const proxyFetcher = new ProxyFetcher();
    const result = await proxyFetcher.autoFetchAndTest(150); // Test up to 150 proxies
    
    res.json({
      success: true,
      message: `Found ${result.working} working proxies out of ${result.totalTested}`,
    ...
    console.error('❌ [SERVER] Error fetching proxies:', error);
    res.status(500).json({ 
      success: false, 
      error: error.message,
      timestamp: new Date().toISOString()
...
// Refresh existing proxies
app.post('/api/refresh-proxies', async (req, res) => {
  try {
    console.log('🔄 [SERVER] Refreshing proxy list...');
    
    const proxyManager = new ProxyManager();
    await proxyManager.loadProxiesFromFile('proxies.txt');
    const testResults = await proxyManager.validateAllProxies();
    
    res.json({
      success: true,
      message: `Proxy refresh completed: ${testResults.working}/${testResults.total} working`,
      ...testResults,
      timestamp: new Date().toISOString()
    });
    
  } catch (error) {
    console.error('❌ [SERVER] Error refreshing proxies:', error);
    res.status(500).json({ 
      success: false, 
      error: error.message,
      timestamp: new Date().toISOString()
...
app.post('/api/test-proxy', async (req, res) => {
  try {
    ...
    console.log(`🔍 [SERVER] Testing proxy: ${proxy}`);
    
    const proxyFetcher = new ProxyFetcher();
    const result = await proxyFetcher.quickTest(proxy);
    ...
    console.error('❌ [SERVER] Error testing proxy:', error);
    res.status(500).json({ 
      success: false, 
      error: error.message,
      timestamp: new Date().toISOString()
...
// Get proxy stats (compatible with frontend)
app.get('/api/proxy-stats', async (req, res) => {
  try {
    const proxyManager = new ProxyManager();
    const fileExists = fsSync.existsSync('proxies.txt');
    
    if (!fileExists) {
      return res.json({ 
        success: true, 
        fileExists: false,
        message: 'No proxy file found',
        totalProxies: 0,
        validProxies: 0,
        successRate: 0
      });
...
// Serve proxies.txt file directly
app.get('/proxies.txt', (req, res) => {
  const filePath = path.join(__dirname, 'proxies.txt');
  
  if (!fsSync.existsSync(filePath)) {
    // Create empty file if doesn't exist
    fsSync.writeFileSync(filePath, '# Auto-fetched proxies\n');
  }
...
// Helper: Get browser launch options untuk multi-browser
async function getBrowserLaunchOptions(config, sessionId) {
  ...
  const platformType = config.platformType || 'windows';
  
  console.log(`[${sessionId}] Browser: ${browserType}, Device: ${deviceType}, Platform: ${platformType}`);
  
  // Base args untuk semua browser
  const baseArgs = [
    '--no-sandbox',
  ...
  let executablePath;
  let product = 'chrome'; // default untuk puppeteer
  ...
  if (browserType === 'firefox') {
    ...
    if (isRailway) {
      ...
      // Untuk local development, install puppeteer-firefox atau gunakan system firefox
      executablePath = process.env.FIREFOX_PATH || 'firefox';
    }
    
    // Firefox-specific args
    const firefoxArgs = [
      ...baseArgs.filter(arg => !arg.includes('disable-accelerated-2d-canvas')),
      '--new-instance'
    ];
    ...
    return {
      product: product,
      args: firefoxArgs,
      defaultViewport: getViewportForDevice(deviceType, config),
    ...
    // WebKit configuration (Safari)
    product = 'webkit';
    console.log(`[${sessionId}] WebKit browser selected - currently using Chromium as fallback`);
    // Fallback to Chromium for now
    browserType = 'chromium';
  }
  
  // Chromium configuration (default)
  if (isRailway) {
    executablePath = process.env.CHROMIUM_PATH || '/usr/bin/chromium';
  } else {
  ...
  // Add window size based on device
  const viewport = getViewportForDevice(deviceType, config);
  chromiumArgs.push(`--window-size=${viewport.width},${viewport.height}`);
  
  // Add device emulation if mobile/tablet
  if (deviceType === 'mobile' || deviceType === 'tablet') {
    chromiumArgs.push('--enable-viewport');
  }
...
// Helper: Get viewport untuk device yang berbeda
function getViewportForDevice(deviceType, config) {
  let viewport;
  
  // Gunakan preset jika ada
  if (config.viewportPreset && config.viewportPreset !== 'auto') {
    viewport = getViewportFromPreset(config.viewportPreset);
  } else {
    // Auto berdasarkan device type
    switch (deviceType) {
      case 'mobile':
  ...
  // Apply realistic viewports jika diaktifkan
  if (config.useRealisticViewports !== false) {
    ...
    viewport.width = Math.max(320, viewport.width + randomWidth);
    viewport.height = Math.max(480, viewport.height + randomHeight);
  }
  
  return viewport;
}

// Helper: Get viewport dari preset
...
async function setupProxy(config, sessionId, socketId, sessionIndex) {
  ...
  // OPTION 3: Multiple manual proxies (dipisah baris)
    ...
    if (proxyList.length === 0) {
      ...
      throw new Error('Proxy list kosong. Format: host:port\nhost:port:user:pass');
    }
    ...
    if (parts.length === 4) {
      ...
      throw new Error(`Format proxy tidak valid. Gunakan "host:port" atau "host:port:user:pass" per baris`);
    }
  }
...
// Fungsi utama untuk menjalankan sesi bot dengan multi-browser support
async function runBotSession(config, sessionIndex, mainSessionId, socketId = null) {
  ...
  try {
    ...
    // 2. Setup User Agent berdasarkan device dan platform
    const deviceType = config.deviceType || 'desktop';
    const platformType = config.platformType || 'windows';
    
    // Handle random selections
    let actualDeviceType = deviceType;
    let actualPlatformType = platformType;
    
    if (deviceType === 'random') {
      const devices = ['desktop', 'mobile', 'tablet'];
      actualDeviceType = devices[Math.floor(Math.random() * devices.length)];
      console.log(`[${sessionId}] Random device selected: ${actualDeviceType}`);
    }
    
    if (platformType === 'random') {
      let platforms;
      if (actualDeviceType === 'desktop') {
        platforms = ['windows', 'macos', 'linux'];
      } else if (actualDeviceType === 'mobile') {
        platforms = ['android', 'ios'];
      } else {
        platforms = ['android', 'ios'];
      }
      actualPlatformType = platforms[Math.floor(Math.random() * platforms.length)];
      console.log(`[${sessionId}] Random platform selected: ${actualPlatformType}`);
    }
    
    // Get user agent untuk session ini
    if (config.rotateUA !== false) {
      const userAgent = userAgentRotator.getUserAgentForSession(sessionIndex, actualDeviceType, actualPlatformType);
      sessionConfig.userAgent = userAgent;
      console.log(`[${sessionId}] Using User Agent: ${userAgent.substring(0, 60)}...`);
      
      emitStatus(socketId, {
        sessionId,
        status: 'browser_config',
        message: `Using ${actualDeviceType} ${actualPlatformType} with ${config.browserType}`,
        progress: 30
      });
    }
    ...
    // 5. Get Browser Launch Options dengan multi-browser support
    ...
    emitStatus(socketId, {
      ...
      message: `Launching ${config.browserType} browser for ${actualDeviceType}...`,
      progress: 40
    });

    console.log(`[${sessionId}] Launching ${config.browserType} browser with ${launchOptions.headless ? 'headless' : 'headed'} mode...`);
    
    try {
      browser = await puppeteer.launch(launchOptions);
      console.log(`[${sessionId}] Browser launched successfully`);
    } catch (launchError) {
      console.error(`[${sessionId}] Failed to launch with ${config.browserType}:`, launchError.message);
      
      emitStatus(socketId, {
        sessionId,
        status: 'warning',
        message: `Trying fallback to Chromium...`,
        progress: 40
      });
      ...
      const fallbackOptions = {
        ...
        headless: 'new',
        timeout: 60000,
        defaultViewport: getViewportForDevice(actualDeviceType, config)
      };
      
      browser = await puppeteer.launch(fallbackOptions);
      console.log(`[${sessionId}] Browser launched with fallback method (Chromium)`);
    }
    ...
    // Set viewport berdasarkan device
    const viewport = getViewportForDevice(actualDeviceType, sessionConfig);
    await page.setViewport(viewport);
    console.log(`[${sessionId}] Viewport set to ${viewport.width}x${viewport.height}`);
    ...
    page.setDefaultNavigationTimeout(config.browserTimeout * 1000 || 30000);
    page.setDefaultTimeout(config.browserTimeout * 1000 || 30000);
    
    // Enable/disable JavaScript jika dikonfigurasi
    if (config.enableJavascript === false) {
      await page.setJavaScriptEnabled(false);
      console.log(`[${sessionId}] JavaScript disabled`);
    }
    
    // Enable/disable images jika dikonfigurasi
    if (config.loadImages === false) {
      ...
      console.log(`[${sessionId}] Images disabled`);
    }
    ...
    return { 
      success: true, 
      sessionId, 
      device: actualDeviceType,
      platform: actualPlatformType,
    ...
    for (let i = 0; i < sessionCount; i++) {
      try {
        emitStatus(socketId, {
          ...
          message: `Memulai sesi ${i + 1}/${sessionCount} (${config.browserType}, ${config.deviceType})`,
...
app.get('/api/active-sessions', (req, res) => {
  const sessions = Array.from(activeSessions.entries()).map(([id, data]) => ({
    ...
    config: data.config ? {
      ...
      browserType: data.config.browserType,
      deviceType: data.config.deviceType,
      platformType: data.config.platformType,
...
io.on('connection', (socket) => {
  ...
  socket.emit('connected', { 
    socketId: socket.id,
    message: 'Connected to Traffic Bot Server v3.2 (Multi-Browser)',
    timestamp: new Date().toISOString(),
    features: {
      multiBrowser: true,
      multiDevice: true,
  ...
  socket.on('start-bot', async (config, acknowledge) => {
    ...
    console.log('[SERVER] Config:', {
      ...
      platformType: config.platformType || 'windows',
    ...
    // Validasi browser type
    if (config.browserType && !['chromium', 'firefox', 'webkit'].includes(config.browserType)) {
      console.error(`[SERVER] ❌ ERROR: Invalid browser type: ${config.browserType}`);
      socket.emit('bot-error', { 
        error: 'Invalid browser type. Supported: chromium, firefox',
        timestamp: new Date().toISOString()
      });
      return;
    }
  ...
  // Proxy management via socket
  socket.on('fetch-proxies', async () => {
    console.log(`[SOCKET-${socket.id}] Fetch proxies requested`);
    
    try {
      const proxyFetcher = new ProxyFetcher();
      const result = await proxyFetcher.autoFetchAndTest(100);
      
      socket.emit('proxy-fetch-complete', {
        success: result.success,
        working: result.working,
        total: result.totalTested,
        message: `Found ${result.working} working proxies`,
        timestamp: new Date().toISOString()
      });
    } catch (error) {
      socket.emit('proxy-fetch-error', {
        error: error.message,
        timestamp: new Date().toISOString()
      });
    }
  ...
  socket.on('refresh-proxies', async () => {
    console.log(`[SOCKET-${socket.id}] Refresh proxies requested`);
    ...
    try {
      ...
      socket.emit('proxy-refresh-complete', {
        success: true,
        working: results.working,
        total: results.total,
        message: `Proxy refresh: ${results.working}/${results.total} working`,
        timestamp: new Date().toISOString()
      });
    } catch (error) {
      socket.emit('proxy-refresh-error', {
        error: error.message,
        timestamp: new Date().toISOString()
      });
    }
...
const serverInstance = server.listen(PORT, HOST, () => {
  ...
  console.log(`
  🚀 TRAFFIC BOT v3.2 SERVER STARTED
  ===================================
  📡 Port: ${PORT}
  🏠 Host: ${HOST}
  ...
  🔌 Socket.IO: Enabled
  🌐 Multi-Browser: Enabled
  📱 Multi-Device: Enabled
  🔧 Auto-Proxy: Enabled
  ✅ Health: http://${HOST}:${PORT}/health
  ✅ Debug: http://${HOST}:${PORT}/api/debug
  ✅ Browser Info: http://${HOST}:${PORT}/api/browser-info
  ✅ Proxy Fetch: http://${HOST}:${PORT}/api/fetch-proxies
  ===================================
  `);
  ...
  if (isRailway) {
    ...
    console.log(`✅ Firefox path: ${process.env.FIREFOX_PATH || '/usr/bin/firefox'}`);
  }
  
  // Log available features
  console.log('✅ Available Browsers: Chromium, Firefox');
  console.log('✅ Device Types: Desktop, Mobile, Tablet');
  console.log('✅ Platforms: Windows, macOS, Linux, Android, iOS');
  
  // Log periodic heartbeat
  setInterval(() => {
    console.log(`❤️  [HEARTBEAT] Uptime: ${process.uptime().toFixed(1)}s | Clients: ${io.engine.clientsCount} | Sessions: ${activeSessions.size}`);
  }, 30000);
});
