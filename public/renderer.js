...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Advanced elements (if they exist)
    ...
    // Load behavior profiles for advanced mode
        ...
        fetch('/api/behavior-profiles')
            ...
            .then(data => {
                if (data.success && data.profiles) {
                    ...
                    behaviorSelect.innerHTML = '<option value="auto">Auto (Weighted Random)</option>' +
                    ...
                    console.log('Loaded behavior profiles:', data.profiles.length);
                }
            })
            .catch(error => {
    ...
    // Initialize advanced controls if they exist
    if (advancedToggle && advancedPanel) {
        ...
        advancedToggle.addEventListener('change', function() {
            ...
            if (this.checked) {
                addLog('Advanced mode enabled', 'info');
                addLog('Behavior profiles loaded', 'info');
            } else {
                addLog('Advanced mode disabled', 'info');
            }
        });
    ...
    startBtn.addEventListener('click', async function() {
        ...
        if (advancedToggle && advancedToggle.checked) {
            const behaviorMode = behaviorSelect ? behaviorSelect.value : 'auto';
            const targetImpressionsVal = targetImpressions ? parseInt(targetImpressions.value) || 20 : 20;
            const targetActiveViewVal = targetActiveView ? parseInt(targetActiveView.value) || 15000 : 15000;
            
            advancedOptions = {
                behaviorMode,
                targetImpressions: targetImpressionsVal,
                targetActiveView: targetActiveViewVal,
                maxDuration: 60000,
                useAdvanced: true
            };
        ...
        try {
            ...
            if (data.success) {
                ...
                if (advancedOptions.useAdvanced) {
                    addLog(`Advanced mode: ${advancedOptions.behaviorMode} profile`, 'info');
                    addLog(`Target: ${advancedOptions.targetImpressions} impressions, ${advancedOptions.targetActiveView}ms active view`, 'info');
                }
            } else {
