document.addEventListener('DOMContentLoaded', function() {
    console.log("🚀 Traffic Bot Dashboard v3.2 (Multi-Browser & Multi-Device) initialized");
    ...
    const platformTypeRadios = document.querySelectorAll('input[name="platformType"]');
    const rotateBrowser = document.getElementById('rotateBrowser');
    const useRealisticViewports = document.getElementById('useRealisticViewports');
    ...
    const availableUAs = document.getElementById('availableUAs');
    const browserStatus = document.getElementById('browserStatus');
    const browserStatusText = document.getElementById('browserStatusText');
    
    // Existing elements (keep all from original)
    ...
    const previewUserAgent = document.getElementById('previewUserAgent');
    ...
    function initializeEventListeners() {
        ...
        browserType.addEventListener('change', updateBrowserConfig);
        
        deviceTypeRadios.forEach(radio => {
            radio.addEventListener('change', updateDeviceConfig);
        });
        
        platformTypeRadios.forEach(radio => {
            radio.addEventListener('change', updatePlatformConfig);
        });
        
        rotateBrowser.addEventListener('change', function() {
            appState.browserConfig.rotate = this.checked;
            log(`Browser rotation: ${this.checked ? 'enabled' : 'disabled'}`, 'info');
        });
        
        useRealisticViewports.addEventListener('change', function() {
            appState.browserConfig.realisticViewports = this.checked;
            updateViewportPreview();
        });
        
        viewportPreset.addEventListener('change', updateViewportFromPreset);
        
        // Preview modal buttons
        document.getElementById('previewDeviceBtn')?.addEventListener('click', showDevicePreview);
        changeDeviceBtn?.addEventListener('click', randomizeDeviceConfig);
        applyDeviceBtn?.addEventListener('click', applyDevicePreview);
        
        // Close device preview modal
        devicePreviewModal?.addEventListener('click', function(e) {
            if (e.target === devicePreviewModal) {
                devicePreviewModal.classList.add('hidden');
            }
        });
        
        document.addEventListener('keydown', function(e) {
            if (e.key === 'Escape' && !devicePreviewModal.classList.contains('hidden')) {
                devicePreviewModal.classList.add('hidden');
            }
        });
    ...
    function initializeBrowserConfig() {
        // Set default browser configuration
        appState.browserConfig = {
            type: 'chromium',
            device: 'desktop',
            platform: 'windows',
            viewport: getViewportFromPreset('auto'),
            rotate: rotateBrowser.checked,
            realisticViewports: useRealisticViewports.checked,
            userAgents: [],
            currentUA: ''
        };
        ...
        log('Browser configuration initialized', 'success');
        log(`Available browsers: Chromium, Firefox, Safari (WebKit)`, 'info');
        log(`Device types: Desktop, Mobile, Tablet`, 'info');
        log(`Platforms: Windows, macOS, Linux, Android, iOS`, 'info');
    }
    
    function loadUserAgents() {
        // Default user agents database
        const defaultUAs = {
            desktop: {
                windows: [
        ...
        appState.userAgents = defaultUAs;
        
        // Calculate total user agents
        let totalUAs = 0;
        Object.keys(appState.userAgents).forEach(device => {
            Object.keys(appState.userAgents[device]).forEach(platform => {
                totalUAs += appState.userAgents[device][platform].length;
            });
        });
        
        availableUAs.textContent = `${totalUAs}+`;
        
        console.log(`📊 Loaded ${totalUAs} user agents`);
    }
    
    function updateBrowserConfig() {
        const browser = browserType.value;
        appState.browserConfig.type = browser;
        
        // Update browser status badge
        if (browser === 'chromium') {
            browserStatus.className = 'proxy-status-badge badge-success';
            browserStatus.textContent = 'Browser: Chromium';
            browserStatusText.textContent = 'Browsers: Chromium available';
        } else if (browser === 'firefox') {
            browserStatus.className = 'proxy-status-badge badge-warning';
            browserStatus.textContent = 'Browser: Firefox';
            browserStatusText.textContent = 'Browsers: Firefox available';
        } else if (browser === 'webkit') {
            browserStatus.className = 'proxy-status-badge badge-info';
            browserStatus.textContent = 'Browser: Safari';
            browserStatusText.textContent = 'Browsers: Safari (WebKit) available';
        }
        
        currentBrowser.textContent = browser.charAt(0).toUpperCase() + browser.slice(1);
        log(`Browser changed to: ${browser}`, 'info');
    }
    ...
    function updateDeviceConfig() {
        ...
        appState.browserConfig.device = device;
        currentDevice.textContent = device.charAt(0).toUpperCase() + device.slice(1);
        
        // Update viewport based on device
        if (useRealisticViewports.checked) {
            updateViewportFromDevice(device);
        }
        
        log(`Device type changed to: ${device}`, 'info');
    }
    
    function updatePlatformConfig() {
        const platform = document.querySelector('input[name="platformType"]:checked')?.value;
        if (!platform) return;
        
        appState.browserConfig.platform = platform;
        currentPlatform.textContent = platform === 'macos' ? 'macOS' : 
                                     platform === 'ios' ? 'iOS' : 
                                     platform.charAt(0).toUpperCase() + platform.slice(1);
        
        log(`Platform changed to: ${platform}`, 'info');
    }
    
    function updateViewportFromDevice(device) {
        let viewport;
        ...
        appState.browserConfig.viewport = viewport;
        updateViewportPreview();
    }
    
    function updateViewportFromPreset() {
        const preset = viewportPreset.value;
        appState.browserConfig.viewport = getViewportFromPreset(preset);
        updateViewportPreview();
    }
    
    function getViewportFromPreset(preset) {
        switch (preset) {
            case 'desktop_1080':
            ...
            case 'mobile_iphone':
                return { width: 390, height: 844, isMobile: true, hasTouch: true };
            case 'mobile_samsung':
                return { width: 360, height: 800, isMobile: true, hasTouch: true };
            case 'mobile_pixel':
                return { width: 393, height: 851, isMobile: true, hasTouch: true };
            case 'tablet_ipad':
                return { width: 1024, height: 1366, isMobile: true, hasTouch: true };
            case 'tablet_samsung':
                return { width: 800, height: 1280, isMobile: true, hasTouch: true };
            case 'auto':
            default:
                const device = appState.browserConfig.device;
                if (device === 'mobile') {
                    return { width: 375, height: 667, isMobile: true, hasTouch: true };
                } else if (device === 'tablet') {
                    return { width: 768, height: 1024, isMobile: true, hasTouch: true };
                } else {
                    return { width: 1920, height: 1080 };
                }
        }
    ...
    function updateViewportPreview() {
        const viewport = appState.browserConfig.viewport;
        previewResolution.textContent = `${viewport.width}x${viewport.height}`;
        
        // Update device frame styling
        if (viewport.isMobile) {
            deviceFrame.style.width = '280px';
            deviceFrame.style.height = '560px';
            deviceScreen.style.width = '250px';
            deviceScreen.style.height = '500px';
        } else if (appState.browserConfig.device === 'tablet') {
            deviceFrame.style.width = '400px';
            deviceFrame.style.height = '550px';
            deviceScreen.style.width = '360px';
            deviceScreen.style.height = '480px';
        } else {
            deviceFrame.style.width = '500px';
            deviceFrame.style.height = '300px';
            deviceScreen.style.width = '460px';
            deviceScreen.style.height = '260px';
        }
    }
    
    function updateBrowserUI() {
        currentBrowser.textContent = appState.browserConfig.type.charAt(0).toUpperCase() + appState.browserConfig.type.slice(1);
        currentDevice.textContent = appState.browserConfig.device.charAt(0).toUpperCase() + appState.browserConfig.device.slice(1);
        currentPlatform.textContent = appState.browserConfig.platform === 'macos' ? 'macOS' : 
                                     appState.browserConfig.platform === 'ios' ? 'iOS' : 
                                     appState.browserConfig.platform.charAt(0).toUpperCase() + appState.browserConfig.platform.slice(1);
    }
    
    function showDevicePreview() {
        // Update preview with current config
        previewDeviceType.textContent = appState.browserConfig.device.charAt(0).toUpperCase() + appState.browserConfig.device.slice(1);
        previewPlatform.textContent = appState.browserConfig.platform === 'macos' ? 'macOS' : 
                                     appState.browserConfig.platform === 'ios' ? 'iOS' : 
                                     appState.browserConfig.platform.charAt(0).toUpperCase() + appState.browserConfig.platform.slice(1);
        
        // Get a sample user agent
        const device = appState.browserConfig.device === 'random' ? 'desktop' : appState.browserConfig.device;
        const platform = appState.browserConfig.platform === 'random' ? 'windows' : appState.browserConfig.platform;
        
        const uaList = appState.userAgents[device]?.[platform] || appState.userAgents.desktop.windows;
        if (uaList.length > 0) {
            previewUserAgent.textContent = uaList[0].substring(0, 50) + '...';
        }
    ...
    function randomizeDeviceConfig() {
        // Randomize device type
        const devices = ['desktop', 'mobile', 'tablet'];
        const randomDevice = devices[Math.floor(Math.random() * devices.length)];
        
        // Randomize platform based on device
        let platforms;
        if (randomDevice === 'desktop') {
            platforms = ['windows', 'macos', 'linux'];
        } else if (randomDevice === 'mobile') {
        ...
        // Update UI
        document.querySelector(`input[name="deviceType"][value="${randomDevice}"]`).checked = true;
        document.querySelector(`input[name="platformType"][value="${randomPlatform}"]`).checked = true;
        ...
        toastr.info(`Randomized: ${randomDevice} ${randomPlatform}`);
    }
    
    function applyDevicePreview() {
        // Update main config from preview
        updateDeviceConfig();
        updatePlatformConfig();
        devicePreviewModal.classList.add('hidden');
        toastr.success('Device configuration applied');
    }
    
    function getUserAgentForSession(sessionIndex) {
        const rotateUA = document.getElementById('rotateUA').checked;
        let device = appState.browserConfig.device;
        let platform = appState.browserConfig.platform;
        ...
        // Get user agent list for the selected device and platform
        let uaList = appState.userAgents[device]?.[platform];
        
        // Fallback to desktop windows if no user agents found
        if (!uaList || uaList.length === 0) {
            uaList = appState.userAgents.desktop.windows;
        }
        
        // Select user agent based on session index
        let uaIndex;
        if (rotateUA) {
            uaIndex = sessionIndex % uaList.length;
        } else {
            uaIndex = 0;
        }
        
        const userAgent = uaList[uaIndex];
        appState.browserConfig.currentUA = userAgent;
    ...
    function initializeSocketEvents() {
        ...
        socket.on('connected', function(data) {
            ...
            if (data.features && data.features.multiBrowser) {
                log('Multi-Browser feature enabled on server', 'success');
            }
        });
        ...
        socket.on('browser-status', function(data) {
            console.log("🌐 Browser Status Event:", data);
            if (data.availableBrowsers) {
                log(`Available browsers: ${data.availableBrowsers.join(', ')}`, 'info');
            }
        });
    ...
    function loadQuickConfig(config) {
        ...
        if (config.browserType) {
            document.getElementById('browserType').value = config.browserType;
            browserType.dispatchEvent(new Event('change'));
        }
        
        if (config.deviceType) {
            document.querySelector(`input[name="deviceType"][value="${config.deviceType}"]`).checked = true;
            updateDeviceConfig();
        }
        
        if (config.platformType) {
            document.querySelector(`input[name="platformType"][value="${config.platformType}"]`).checked = true;
            updatePlatformConfig();
        }
        
        if (config.rotateUA !== undefined) document.getElementById('rotateUA').checked = config.rotateUA;
        if (config.rotateBrowser !== undefined) document.getElementById('rotateBrowser').checked = config.rotateBrowser;
        if (config.useRealisticViewports !== undefined) document.getElementById('useRealisticViewports').checked = config.useRealisticViewports;
        if (config.viewportPreset) document.getElementById('viewportPreset').value = config.viewportPreset;
        if (config.enableJavascript !== undefined) document.getElementById('enableJavascript').checked = config.enableJavascript;
        if (config.loadImages !== undefined) document.getElementById('loadImages').checked = config.loadImages;
    ...
    function gatherConfig() {
        ...
        const config = {
            ...
            rotateBrowser: document.getElementById('rotateBrowser').checked,
            useRealisticViewports: document.getElementById('useRealisticViewports').checked,
    ...
    setInterval(() => {
        browserStatusText.textContent = `Browsers: ${appState.browserConfig.type} (${appState.browserConfig.device} ${appState.browserConfig.platform})`;
    }, 30000);
    
    // Initial log
    log('Multi-Browser & Multi-Device Traffic Bot Dashboard initialized successfully', 'success');
    log('Supported browsers: Chromium, Firefox, Safari (WebKit)', 'info');
    log('Device types: Desktop, Mobile, Tablet', 'info');
    log('Platforms: Windows, macOS, Linux, Android, iOS', 'info');
