document.addEventListener('DOMContentLoaded', function() {
    ...
    function startBotSessions() {
        console.log("🔍 DEBUG: startBotSessions dipanggil");
        
        const config = gatherConfig();
        console.log("🔍 DEBUG: Config gathered:", JSON.stringify(config, null, 2));
        
        // VALIDASI KHUSUS WEBSITE
        if (config.target === 'website') {
            console.log("🔍 DEBUG: Validasi website traffic...");
            
            // Validasi URL
            if (!config.webUrl || config.webUrl.trim() === '') {
                console.error("❌ ERROR: Website URL kosong");
            ...
            if (!config.webUrl.startsWith('http://') && !config.webUrl.startsWith('https://')) {
                ...
                console.log("🔍 DEBUG: URL ditambahkan https://:", config.webUrl);
            }
            
            console.log("✅ Website config valid:", {
                url: config.webUrl,
                duration: config.visitDuration,
                sessions: config.sessionCount
            });
        }
        ...
        console.log("🔍 DEBUG: Mengirim config ke server via socket.emit...");
        
        // DEBUG: Cek socket connection
        ...
        console.log("🔍 DEBUG: Event 'start-bot' telah dikirim");
        
        // Fallback: Jika tidak ada respon dalam 5 detik
        setTimeout(() => {
            if (appState.isRunning && appState.activeSessionCount === config.sessionCount) {
                console.warn("⚠️ WARNING: No response from server after 5 seconds");
