document.addEventListener('DOMContentLoaded', function() {
    ...
    // Force reconnection jika tidak terkoneksi
    setTimeout(() => {
        if (!socket.connected) {
    ...
    function initializeSocketEvents() {
        ...
        socket.on('error', (error) => {
            ...
            toastr.error('Socket connection error');
        });
    ...
    function startBotSessions() {
        ...
        // Reset UI state terlebih dahulu
        ...
        // Validate configuration - TAMPILKAN ERROR LEBIH DETAIL
        ...
        if (validationErrors.length > 0) {
            ...
            // Reset UI jika validasi gagal
            resetControls();
            return;
        }
        
        // Normalize URL jika needed
        ...
        if (!socket.connected) {
            ...
            toastr.error("Not connected to server. Trying to reconnect...");
            ...
            setTimeout(() => {
                if (!socket.connected) {
                    ...
                    setTimeout(() => startBotSessions(), 1000);
                }
            }, 2000);
            return;
        }
        
        console.log("✅ Socket connected, emitting start-bot event");
        
        // Send config to server
        socket.emit('start-bot', config, function(ack) {
            console.log("📩 Server acknowledged:", ack);
            if (!ack) {
                log("Warning: No acknowledgement from server", 'warning');
            }
        });
        
        // Set timeout untuk check status
        setTimeout(() => {
            if (appState.isRunning && appState.activeSessionCount === config.sessionCount) {
                console.log("⏳ Checking if bot started...");
                log("Waiting for bot to start...", 'info');
            }
        }, 3000);
    }
